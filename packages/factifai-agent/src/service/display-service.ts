...
import stringWidth from "string-width"; // For accurate width calculation of terminal strings with ANSI codes
...
export class ConsoleDisplayService {
  ...
  private calculateLines(str: string): number {
    ...
    const width = typeof stringWidth === 'function' ? 
  ...
  private calculateTotalLines(content: string[]): number {
    return content.reduce((total, line) => total + this.calculateLines(line), 0);
  }
  ...
  private clearTrackActionDisplay(): void {
    if (this.trackActionActive && this.lastTrackActionLines > 0 && !this.isResizing) {
      try {
        ...
        this.originalConsoleLog.call(console, 
          chalk.red(`Display service error: ${error instanceof Error ? error.message : String(error)}`)
        );
      }
    }
  }
  ...
  public updateTrackActionDisplay(content: string[]): void {
    ...
    try {
      ...
      this.originalConsoleError.call(console, 
        `Display service error: ${error instanceof Error ? error.message : String(error)}`
      );
    } finally {
